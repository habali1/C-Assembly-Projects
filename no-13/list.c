...
void list_insertOrder(list lst,node insert) {
	assert(list_valid(lst));
	assert(node_getNext(insert) == NULL);
        if (*lst == NULL || node_getPayload(*lst) >= node_getPayload(insert) ) {
	  node_setNext (insert,*lst);
	  *lst = insert;
	}
	else{
	  ...
	  node prev = *lst;
	  node current = node_getNext(*lst);
	  while (current != NULL && node_getPayload (current) < node_getPayload (insert)) {
	    prev = current;
	    current = node_getNext (current);
	  }
	  node_setNext (insert, current);
